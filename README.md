# 이재혁 | 데이터 분석가 포트폴리오

## 👤 프로필 정보

<table>
  <tr>
    <td align="center" width="200">
      <img src="이재혁.jpg" width="180" alt="프로필 사진" />
    </td>
    <td style="padding-left: 20px;">
      <br>
      <b>이름</b> : 이재혁<br><br>
      <b>생년월일</b> : 1999.06.21 (만 26세)<br><br>
      <b>연락처</b> : 010.6691.0864<br><br>
      <b>Email</b> : <a href="mailto:wogur4523@naver.com">wogur4523@naver.com</a>
    </td>
  </tr>
</table>

---

데이터를 분석해 문제를 이해하고, 의미 있는 결과를 도출하는 과정을 중요하게 생각합니다.

AIA생명 T/M 실무에서 특정 스크립트에 대한 연령대별 전환율 차이를 직접 경험하며,  
고객 반응을 데이터로 설명하고 세분화해야 한다는 문제의식을 갖게 되었습니다.

이후 컴퓨터공학 전공 지식과 데이터 분석 교육을 기반으로  
데이터 전처리, 모델링, 시각화 전 과정을 학습했습니다.

분석 결과가 실제 업무에 도움이 될 수 있도록 데이터를 해석하고 전달하는 분석가를 목표로 하고 있습니다.

---

## 🎓 학력

- 호남대학교 컴퓨터공학과 졸업 (2018.03 - 2025.02)

## 💻 교육

- 멀티캠퍼스 데이터 분석가 부트캠프 수료 (2025.11.17 - 2026.04.16)

학습 내용
- Python, SQL 기반 데이터 수집·전처리, 통계 분석 및 시각화 역량 습득
- 머신러닝 기초 모델을 활용한 예측 분석 및 데이터 분석 프로젝트 수행

## 💼 경력 사항

- AIA생명 T/M 아웃바운드 마케팅 (2021.01 - 2021.12)
- 고객 응대 및 서비스 직무 경력 5년 이상 (CU, PC방, 외식업 등)

---

## 🛠 기술 스택

### Language & Data
<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white"/>
  <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white"/>
  <img src="https://img.shields.io/badge/Numpy-013243?style=for-the-badge&logo=numpy&logoColor=white"/>
</p>

### Machine Learning
<p>
  <img src="https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white"/>
  <img src="https://img.shields.io/badge/LightGBM-02569B?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/CatBoost-FFCC00?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/XGBoost-EC1C24?style=for-the-badge"/>
</p>

### Visualization & Collaboration
<p>
  <img src="https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit&logoColor=white"/>
  <img src="https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/SHAP-FF6F61?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  <img src="https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white"/>
</p>

---

### 1. TVING 사용자 행동 데이터 기반 고객 이탈 위험 예측 및 개입 전략 분석 (팀 프로젝트)

📅 프로젝트 기간 : 2026.02.05 ~ 2026.04.14

사용자 로그 데이터를 활용하여 이탈(Churn)을 사전 예측하고, 마케팅 개입을 위한 고객 페르소나를 도출한 프로젝트입니다.

### 나의 역할 및 성과
- Churn proxy 재정의 및 7일 snapshot / 28일 feature window / 14일 label window 분석 구조 설계
- 모델 비교 분석을 통해 성능과 해석력이 우수한 LightGBM을 최종 모델로 선정
- LightGBM 성능 지표: PR-AUC 0.859, Precision@10 1.000, Lift@10 1.372 달성
- SHAP를 통해 시청 시간, 접속 빈도 등 핵심 이탈 변수 식별
- 분석 결과 기반 Risk_band 및 Persona_type 도출
- Figma 기반 마케팅 액션 대시보드 프로토타입 설계

사용 기술
- `Python` `pandas` `LightGBM` `CatBoost` `SHAP` `Figma`

🔗 [Repository 바로가기](https://github.com/jaehyeok-7/TVING-Churn-Prediction)

---

### 2. 학생 시험 성적 영향 요인 분석 및 예측 모델링 (개인 프로젝트)

학생들의 생활 패턴(학습 시간, 출석률, 수면 시간 등)이 학업 성취도에 미치는 영향을 분석한 프로젝트입니다.

### 나의 역할 및 성과
- 회귀분석 및 상관관계 분석을 통한 주요 변수 도출
- LightGBM 기반 성적 예측 모델 구축
- SHAP를 활용한 변수 영향도 해석

사용 기술
- `Python` `pandas` `scikit-learn` `LightGBM` `SHAP`

🔗 [Repository 바로가기](https://github.com/jaehyeok-7/2rd-mini-project)


---

### 3. 대사 지표 기반 당뇨 위험 요인 분석 및 진단 예측 (팀 프로젝트)

10만 건 이상의 대사 지표 데이터를 분석하여 당뇨병 발병 위험 요인을 파악하고 진단 모델을 구축했습니다.

### 나의 역할 및 성과
- OLS, ANOVA, 로지스틱 회귀를 활용한 유의미 변수 선별
- 머신러닝 모델 비교 및 성능 최적화 수행

사용 기술
- `Python` `statsmodels` `scikit-learn` `XGBoost` `LightGBM`

🔗 [Repository 바로가기](https://github.com/jaehyeok-7/kaggle_mini_project)

---

## 📫 Contact

- Email: wogur4523@naver.com
- GitHub: https://github.com/jaehyeok-7
